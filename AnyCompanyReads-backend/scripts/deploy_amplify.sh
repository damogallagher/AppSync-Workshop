...
function get_amplify_app() {
    ...
    if [ -z "$APP_ID" ]; then
        echo $(aws amplify create-app --name $APP_NAME --custom-rules '[{"source":"</^[^.]+$|\\.(?!(css|gif|ico|jpg|js|png|txt|svg|woff|woff2|ttf|map|json|webp)$)([^.]+$)/>","target":"/index.html","status":"200"}]' --output text --query "app.appId")
    else
        echo ${APP_ID[0]}
    fi
}
